export const productReducer = (state, action) => {
    switch (action.type) {
        ...
        case 'EDIT_ADDRESS': return {
            ...state, address: state.address.map((addressItem) => addressItem.id === action.payload
                ? { ...addressItem, isEdit: true }
                : addressItem)
        };
        case 'SAVE_EDITED_ADDRESS': return {
            ...state, address: state?.address.map((addressItem) => addressItem.id === action.payload[1]
                ? { ...action.payload[0] }
                : addressItem
            )
        };
